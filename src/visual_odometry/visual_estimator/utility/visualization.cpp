    ...
    else
    {
      ...
      /*  static tf::Transform t_odom_world = tf::Transform(tf::createQuaternionFromRPY(0, 0, 0), tf::Vector3(0, 0, 0));
        if (header.stamp.toSec() - last_align_time > 1.0)
        {
            try
            {
                tf::StampedTransform trans_odom_world;
		listener.waitForTransform("odom","vins_world", ros::Time(0), ros::Duration(3.0));
                listener.lookupTransform("odom","vins_world", ros::Time(0), trans_odom_world);
		t_odom_world=trans_odom_world;
                last_align_time = header.stamp.toSec();
            } 
            catch (tf::TransformException ex){}
        }
